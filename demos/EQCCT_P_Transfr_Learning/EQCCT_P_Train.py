...
def _build_model(args): 
    ...
    # Used only in Transfer Learning
    model.load_weights('./ModelPS/test_trainer_024.h5')
...
def _split(args, save_dir):
    ...
    """       
    # If not passing the train/validation/testing IDs name, use this command to randomly divide the data.
    ...
    training = training[0:int(0.3*len(training))]# for transfer learning only
    # Saving them if you need.
...
def _document_training(history, model, start_training, end_training, save_dir, save_models, training_size, validation_size, args): 
    ...
    with open(os.path.join(save_dir,'X_report.txt'), 'a') as the_file: 
        ...
        the_file.write('last picker_P_loss: '+str(history.history['loss'][-1])+'\n')
        the_file.write('last picker_P_f1: '+str(history.history['f1'][-1])+'\n')
